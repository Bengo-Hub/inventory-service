...
# Build context should be from workspace root: docker build -f inventory-service/Dockerfile -t inventory-service:local .
COPY shared/auth-client /shared/auth-client
COPY inventory-service/go.mod inventory-service/go.sum ./
RUN go mod download
COPY inventory-service .

RUN CGO_ENABLED=0 go build -o /out/inventory ./cmd/api
