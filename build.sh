...
APP_NAME=${APP_NAME:-"inventory-service"}
NAMESPACE=${NAMESPACE:-"inventory"}
ENV_SECRET_NAME=${ENV_SECRET_NAME:-"inventory-service-secrets"}
DEPLOY=${DEPLOY:-true}
SETUP_DATABASES=${SETUP_DATABASES:-true}
DB_TYPES=${DB_TYPES:-postgres,redis}
# Per-service database configuration
SERVICE_DB_NAME=${SERVICE_DB_NAME:-inventory}
SERVICE_DB_USER=${SERVICE_DB_USER:-inventory_user}
...
GIT_USER=${GIT_USER:-"Inventory Bot"}
...
if ! kubectl -n "$NAMESPACE" get secret "$ENV_SECRET_NAME" >/dev/null 2>&1; then
  ...
  kubectl -n "$NAMESPACE" create secret generic "$ENV_SECRET_NAME" \
    --from-literal=INVENTORY_POSTGRES_URL="postgresql://${SERVICE_DB_USER}:PASSWORD@postgresql.infra.svc.cluster.local:5432/${SERVICE_DB_NAME}?sslmode=disable" \
    --from-literal=INVENTORY_REDIS_ADDR="redis-master.infra.svc.cluster.local:6379" \
    --from-literal=INVENTORY_NATS_URL="nats://nats.messaging.svc.cluster.local:4222" || true
fi
